...
spritesLoaded++};img.onerror=()=>{spritesLoaded++}}
...
let S={screen:'intro',party:null,partyData:null,lives:MAX_LIVES,score:0,startTime:0,
bestTime:parseFloat(localStorage.getItem('kk_best')||'0'),speed:INIT_SPEED,
spawnInt:SPAWN_INIT,lastSpawn:0,lastSpeedUp:0,pLane:1,tLane:1,pX:0,pY:0,
...
if(S.muted||!f)return;try{ensureAudio();
...
if(S.muted)return;try{ensureAudio();
// Kick body
const o=actx.createOscillator(),g=actx.createGain();
...
S.speed=INIT_SPEED;S.spawnInt=SPAWN_INIT;S.lastSpawn=0;S.lastSpeedUp=0;
...
// Touch: swipe
canvas.addEventListener('touchstart',e=>{if(S.screen!=='playing')return;
tsX=e.touches[0].clientX;tsY=e.touches[0].clientY;e.preventDefault()},{passive:false});
canvas.addEventListener('touchend',e=>{if(S.screen!=='playing')return;
...
if(!S.disc.has(o.type.id)){S.disc.add(o.type.id);
...
function spawnParts(x,y,c,n){for(let i=0;i<n;i++)S.particles.push({x,y,
...
// Shield glow
if(S.shielded){ctx.shadowColor='#00ff88';ctx.shadowBlur=15;
...
else{ctx.font=OBS_SIZE*0.65+'px serif';ctx.textAlign='center';ctx.textBaseline='middle';
...
const s=4*p.life;ctx.fillRect(p.x-s/2,p.y-s/2,s,s);});ctx.globalAlpha=1;}
...
S.spawnInt=Math.max(SPAWN_MIN,S.spawnInt-SPAWN_DEC);S.lastSpeedUp=now;}
...
function copyTxt(t){navigator.clipboard.writeText(t).then(()=>{
